...
def accuracy(typed, source):
    ...
    if typed==source:
        ...
        score = 0
        roll_num = min(len(source_words),len(typed_words))
...
def minimum_mewtations(typed, source, limit):
    ...
    if source == typed: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 0
        # END
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if times[j][i]<min_time:
                min_time = times[j][i]
                index=j
        output[index]=output[index]+[words[i]]
    return output
    # END PROBLEM 10
